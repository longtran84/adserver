...
public class PhonecardRestController {
    ...
    public ResponseEntity<?> activatePhoneCard(@RequestBody Phonecard phonecard) {
        try {
            String status = phonecard.getStatus().equals("ACTIVE") ? "INACTIVE" : "ACTIVE";
            phonecardService.updateStatus(phonecard.getId(), status);
        } catch (Exception ex) {
            return ResponseEntity.badRequest().body("Error");
        }
        return ResponseEntity.ok(phonecard.getId());
    }
